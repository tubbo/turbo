...
impl AssetReference for PackageJsonReference {
    ...
    fn resolve_reference(&self) -> Vc<ResolveResult> {
        ResolveResult::asset(Vc::upcast(SourceAsset::new(self.package_json))).into()
    }
}
...
async fn extend_with_constant_pattern(
    ...
    result.extend(
        realpath_with_links
            .symlinks
            .iter()
            .map(|l| Vc::upcast(SourceAsset::new(*l))),
    );
    ...
    match &*entry_type {
        ...
        FileSystemEntryType::File | FileSystemEntryType::Symlink => {
            result.insert(Vc::upcast(SourceAsset::new(dest_file_path)));
        }
        _ => {}
    }
    Ok(())
        ...
        for (_, entry) in entries {
            match entry {
                DirectoryEntry::File(file) => {
                    result.insert(Vc::upcast(SourceAsset::new(*file)));
                }
