...
async fn css_chunk_content(
    ...
    let mut all_async_chunk_group_entries = IndexSet::<Vc<Box<dyn Chunk>>>::new();
...
impl Asset for CssChunk {
    ...
    async fn references(self: Vc<Self>) -> Result<Vc<AssetReferences>> {
        ...
        for entry in content.async_chunk_group_entries.iter() {
            references.push(Vc::upcast(ChunkGroupReference::new(this.context, *entry)));
        }
...
impl FromChunkableAsset for Box<dyn CssChunkItem> {
    async fn from_asset(
        context: Vc<Box<dyn ChunkingContext>>,
    ...
    async fn from_async_asset(
        ...
        _asset: Vc<Box<dyn ChunkableAsset>>,
